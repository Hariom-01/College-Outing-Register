...
if (isset($_POST['registration']) && isset($_POST['password'])
	...
	$user_data = 'registration='. $registration. '&name='. $name;


	if (empty($registration)) {
		header("Location: signup.php?error=Registration is required&$user_data");
	    exit();
	}else if(empty($pass)){
        ...
        header("Location: signup.php?error=Class Name is required&$user_data");
	    exit();
	}
		...
		if (mysqli_num_rows($result) > 0) {
           ...
           $sql2 = "INSERT INTO students(registration, password, name,class_name,roll_number) VALUES('$registration', '$pass', '$name','$class_name','$roll_number')";
